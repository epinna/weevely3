...
class Dump(Module):
    ...
    def init(self):
        ...
        self.register_arguments(
            [
                ...
                {
                    "name": "-dbms",
                    "help": "Db type. Vector 'mysqldump_sh' supports only 'mysql'.",
                    "choices": ("mysql", "pgsql", "sqlite", "dblib"),
                    "default": "mysql",
                },
    ...
    def run(self, **kwargs):
        ...
        if self.args["vector"] == "mysqldump_sh" and self.args["dbms"] != "mysql":
            log.warn(messages.module.vector_s_not_support_arg_s_s % (self.args["vector"], "dbms", self.args["dbms"]))
            return

...
class Curl(Module):
    ...
    def init(self):
        ...
        self.register_vectors(
            [
                ...
                PhpFile(
                    payload_path=os.path.join(self.folder, "php_curl.tpl"), name="php_curl", arguments=["-raw-response"]
                ),
                PhpFile(
        ...
        self.register_arguments(
            [
                ...
                {"name": "--connect-timeout", "type": int, "default": 5, "help": "Default: 2"},
                {
                    "name": "--request",
                    "dest": "request",
                    "choices": ("GET", "HEAD", "POST", "PUT", "OPTIONS"),
                    "default": "GET",
                },
                {
                    "name": "-X",
                ...
                },
                {
                    ...
                    "action": "store_true",
                    "default": False,
                    "help": "Save file locally with -o|--output",
                },
    ...
    def run(self, **kwargs):
        ...
        vector_name, result = self.vectors.find_first_result(
            names=[self.args.get("vector")], format_args=self.args, condition=lambda r: r if r and r.strip() else None
        )
        ...
        headers = [h.rstrip() for h in headers.split(b"\r\n")] if b"\r\n" in headers else headers
        ...
        if output_path:
            ...
            if not self.args.get("local"):
                ...
                try:
                    ...
                    log.warning(messages.generic.error_loading_file_s_s % (output_path, str(e)))
                    saved = False
                else:
                    saved = True

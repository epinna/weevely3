...
class FileBzip(BaseTest):
    ...
    def setUp(self):
        ...
        # Create and bzip2 a binary file for the test
        ...
        self.compressed = [ os.path.join(config.script_folder, 'binfile.bz2') ]

        self.check_call(config.cmd_env_content_s_to_s % (self.binary_string, self.uncompressed[0]))
        self.check_call(config.cmd_env_bzip2_s % (self.uncompressed[0]))

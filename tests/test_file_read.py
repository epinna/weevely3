...
BASE_FOLDER="{config.base_folder}/test_read/"
mkdir -p "$BASE_FOLDER"
...
class FileRead(BaseTest):
    ...
    def test_read_php(self):
        ...
        # Simple download
        self.assertEqual(self.run_argv(['test_read/ok.test']), 'OK')
        ...
        self.assertEqual(self.run_argv(['test_read/ko.test']), None)
    ...
    def test_read_allvectors(self):
        ...
        for vect in modules.loaded['file_download'].vectors.get_names():
            self.assertEqual(self.run_argv(['-vector', vect, 'test_read/ok.test']), 'OK')

    def test_read_sh(self):

        # Simple download
        self.assertEqual(self.run_argv(['-vector', 'base64', 'test_read/ok.test']), 'OK')
        ...
        # Download of an unreadable file
        self.assertEqual(self.run_argv(['-vector', 'base64', 'test_read/ko.test']), None)
